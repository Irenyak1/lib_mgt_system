...
router.get('/admin/dashboard', async (req, res) => {
  if (req.session.user) {
    try {
      ...
      let numBorrowedBooks = await BookModel.countDocuments({ status: 'borrowed' });
      ...
      let numAvailableBooks = await BookModel.aggregate([ { $match: { 'status' : 'available' } },
        {$group: { _id: null, // Group by null to calculate across all documents
